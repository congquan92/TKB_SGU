...
export default function App() {
            ...
            if (savedSubjects) {
                ...
                if (Array.isArray(parsed)) {
                    ...
                    return parsed.filter((savedSubject: Subject) => {
                        const originalSubject = data.find((d) => d.ma_mon === savedSubject.ma_mon && d.nhom_to === savedSubject.nhom_to && d.to === savedSubject.to);
    ...
    useEffect(() => {
        if (!ignoreSlotLimit && selected.length > 0) {
            ...
            const validSubjects = selected.filter((subject) => {
                const originalSubject = data.find((d) => d.ma_mon === subject.ma_mon && d.nhom_to === subject.nhom_to && d.to === subject.to);
    ...
    const addSubject = (mon: Subject) => {
        ...
        if (hasConflict.length > 0) {
            toast.error(` Môn ${mon.ma_mon} bị trùng lịch với: ${hasConflict.join(", ")}`, {
                position: "top-center",
                autoClose: 3000,
                hideProgressBar: false,
                closeOnClick: true,
                ...
                if (Array.isArray(arr)) {
                    ...
                    if (isValidData) {
                        ...
                        importedSubjects.forEach((importedSubject) => {
                            const originalSubject = data.find((d) => d.ma_mon === importedSubject.ma_mon && d.nhom_to === importedSubject.nhom_to && d.to === importedSubject.to);
                        ...
                        if (conflicts.length > 0) {
                            toast.error(`File JSON có trùng lịch! Các môn bị trùng: ${conflicts.join(", ")}`, {
                                position: "top-center",
                                autoClose: 3000,
                            });
                            return;
                        ...
                        if (existingConflicts.length > 0) {
                            toast.error(`Một số môn trong file bị trùng lịch với môn đã chọn: ${existingConflicts.join(", ")}`, {
                                position: "top-center",
                                autoClose: 3000,
                            });
                            return;
    ...
    const captureImage = async () => {
        ...
        try {
            ...
            canvas.toBlob(
                (blob: Blob | null) => {
                    if (blob) {
                        ...
                        toast.success(
                            <>
                                <i className="fa-solid fa-face-smile-beam mx-2"></i> Đã chụp ảnh thời khóa biểu thành công!
                            </>,
                            {
                                position: "top-center",
                                autoClose: 2000,
                        ...
                        toast.error(
                            <>
                                <i className="fa-solid fa-circle-xmark mx-2"></i> Không thể tạo file ảnh!
                            </>,
                            {
                                position: "top-right",
                                autoClose: 2000,
            ...
            toast.error(
                <>
                    <i className="fa-solid fa-circle-xmark mx-2"></i> Lỗi khi chụp ảnh! Vui lòng thử lại sau.
                </>,
                {
                    position: "top-center",
                    autoClose: 3000,
    ...
    return (
        <div className="container-fluid px-2 py-3">
                ...
                <div className="d-flex justify-content-between align-items-center">
                    <div className="d-flex align-items-center gap-3">
                        ...
                        <div className="d-none d-sm-flex align-items-center gap-2">
                            <span
                                className="badge subject-count-badge"
                                style={{
                    ...
                    <div className="d-flex align-items-center gap-2 gap-md-3">
                        ...
                        >
                            {theme === "light" ? <i className="fa-solid fa-moon" style={{ fontSize: "16px", color: "var(--background-color)" }}></i> : <i className="fa-solid fa-sun" style={{ fontSize: "16px", color: "var(--background-color)" }}></i>}
                        </div>
            ...
            <div className="row">
                ...
                {/* Sidebar */}
                <div
                    className={`col-lg-2 col-md-4 mb-4 ${showMobileSidebar ? "d-block" : "d-none d-md-block"}`}
                    style={{
                        ...(showMobileSidebar
                            ? {
                                  position: "fixed",
                ...
                >
                            ...
                            <div className="d-flex justify-content-between align-items-center">
                                ...
                                <div className="d-flex align-items-center gap-2">
                                    ...
                                    <div
                                        ...
                                        // }}
                                    >
                                        {ignoreSlotLimit ? <i className="fa-solid fa-lock-open" style={{ fontSize: "14px" }}></i> : <i className="fa-solid fa-lock" style={{ fontSize: "14px" }}></i>}
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="card-body p-3">
                            <SubjectSelector data={data as Subject[]} onSelect={addSubject} ignoreSlotLimit={ignoreSlotLimit} />
                        </div>
                    </div>
                    ...
                    {selected.length > 0 && (
                        ...
                        >
                            ...
                            >
                                <div className="d-flex justify-content-between align-items-center">
                                    <h6 className="mb-0 subject-count-header">
                                        <i className="fa-solid fa-book"></i> Môn học đã chọn ({selected.length})
                                    </h6>
                                    <div
                                        onClick={clearAll}
                ...
                <div className="col-lg-10 col-md-8">
                    ...
                    >
                        ...
                        >
                            <h6 className="mb-0">
                                <i className="fa-solid fa-calendar"></i> Thời khóa biểu ({selected.length} môn - {totalCredits} tín chỉ)
                            </h6>
                        </div>
                        <div className="card-body p-2">
                            ...
                            {selected.length === 0 && (
                                <div className="text-center py-5" style={{ color: "var(--text-secondary)" }}>
                                    <h5>
                                        <i className="fa-solid fa-bookmark"></i> Chưa có môn học nào
                                    </h5>
            ...
            {/* Donate Modal */}
            <DonateModal show={showDonateModal} onHide={() => setShowDonateModal(false)} />

            {/* Toast Container */}
            <ToastContainer position="top-right" autoClose={3000} hideProgressBar={false} newestOnTop={false} closeOnClick rtl={false} pauseOnFocusLoss draggable pauseOnHover theme={theme === "dark" ? "dark" : "light"} />
        </div>
    );
}
